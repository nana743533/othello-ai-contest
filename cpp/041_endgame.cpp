/*
 * CodinGame Othello AI - Path Score + Endgame
 * 空き12以下で終盤完全読み、それ以外は打点合計の先読み
 */
...
/* この空きマス数以下で終盤完全読みに切り替える */
static constexpr int ENDGAME_EMPTY = 12;
...
static int searchPath(
    ...
    if (moves.empty()) {
        ...
        return searchPath(board, depth - 1, 1 - currentPlayer, me, pathScore, alpha, beta);
    }
    ...
    if (currentPlayer == me) {
        ...
        for (const string& move : moves) {
            int nextScore = pathScore + moveDelta(move, currentPlayer, me);
            Board next = applyMove(board, move, currentPlayer);
            int score = searchPath(next, depth - 1, 1 - currentPlayer, me, nextScore, alpha, beta);
...
static int searchEndgame(
    ...
    if (moves.empty()) {
        ...
        if (getLegalMoves(board, 1 - currentPlayer).empty()) {
            return finalScore(board, me);
        }
        return searchEndgame(board, 1 - currentPlayer, me, alpha, beta);
    }

    if (currentPlayer == me) {
        /* 自分の手番: 最終石数差が最大になる手を選ぶ */
        int bestScore = INT_MIN;
        for (const string& move : moves) {
            Board next = applyMove(board, move, currentPlayer);
            int score = searchEndgame(next, 1 - currentPlayer, me, alpha, beta);
            bestScore = max(bestScore, score);
            alpha = max(alpha, bestScore);
            if (beta <= alpha) break;
        }
...
static pair<string, int> choosePathMove(const Board& board, vector<string> legalMoves, int me) {
    ...
    for (const string& move : legalMoves) {
        int pathScore = moveDelta(move, me, me);
        Board next = applyMove(board, move, me);
        int score = searchPath(next, DEPTH - 1, 1 - me, me, pathScore, alpha, beta);
        if (score > bestScore) {
            bestScore = score;
            bestMove = move;
        }
...
/* 終盤: 最終石数差が最大になる初手を選ぶ */
static pair<string, int> chooseEndgameMove(const Board& board, const vector<string>& legalMoves, int me) {
    string bestMove = legalMoves[0];
    int bestScore = INT_MIN;
    int alpha = INT_MIN;
    int beta = INT_MAX;
    ...
    for (const string& move : legalMoves) {
        Board next = applyMove(board, move, me);
        int score = searchEndgame(next, 1 - me, me, alpha, beta);
        if (score > bestScore) {
            bestScore = score;
            bestMove = move;
        }
...
/* 終盤MSGを EG+4 / EG-12 形式にする */
static string formatEgMsg(int score) {
    ostringstream oss;
    oss << "EG" << showpos << score << noshowpos;
    return oss.str();
}

/* 空きマス数に応じて中盤探索か終盤完全読みかを選ぶ */
static pair<string, string> chooseMove(const Board& board, vector<string> legalMoves, int me) {
    if (countEmpty(board) <= ENDGAME_EMPTY) {
        auto [move, score] = chooseEndgameMove(board, legalMoves, me);
        return {move, formatEgMsg(score)};
    }
    auto [move, score] = choosePathMove(board, legalMoves, me);
    return {move, to_string(score)};
}

/* CodinGame 入出力ループ。最善手と評価MSGを出力する */
int main() {
    ...
    /* 毎ターン: 盤面と合法手を読み、中盤/終盤を切り替えて手を選ぶ */
    while (cin) {
        vector<string> rows;
        ...
        auto [move, msg] = chooseMove(board, legalMoves, me);
        cout << move << " MSG " << msg << endl;
    }
